...
public class Mercury : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //GetComponent<Orbits>().radialDistance = 2.0f;
        GetComponent<Orbits>().majorAxis = 3.0f;
        GetComponent<Orbits>().minorAxis = 2.0f;
        GetComponent<Orbits>().speed = 0.009f;
        GetComponent<Orbits>().angle = 0.2f;
        transform.position = new Vector3(0, 0, 0);
        GetComponent<RotateAround>().angle = 0.03f;
    }
    ...
    void OnGUI() //display the name of the planet if the mouse point intersects the object
    {
        if (GetComponent<Mouse>().intersection == true)
            GUI.Box(new Rect(GetComponent<Mouse>().twoD.x, Screen.height - GetComponent<Mouse>().twoD.y, 50, 50), "Mercury"); 
    }
}
