...
public class moonMercury : MonoBehaviour {

    public GameObject motherPlanet; //parent planet
    public Mercury motherScript;    //get access to mother planet's code
    Vector3 position;

    // Use this for initialization
    void Start ()
    {
        motherPlanet = GameObject.Find("Mercury");  //find parent planet
        motherScript = motherPlanet.GetComponent<Mercury>(); //get script
    ...
    void OnGUI()  //display the name of the planet if the mouse point intersects the object
    {
        ...
        if (GetComponent<Mouse>().intersection == true)
            GUI.Box(new Rect(GetComponent<Mouse>().twoD.x, Screen.height - GetComponent<Mouse>().twoD.y, 75, 50), "Mercury Moon");
    }
}
