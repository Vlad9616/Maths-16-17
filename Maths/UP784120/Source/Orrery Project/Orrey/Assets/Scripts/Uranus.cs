...
public class Uranus : MonoBehaviour
{
    ...
    void Start()
    {
      ...
      //  GetComponent<Orbits>().radialDistance = 13.0f;
        GetComponent<Orbits>().majorAxis = 16.0f;
        GetComponent<Orbits>().minorAxis = 14.0f;
        GetComponent<Orbits>().speed = 0.003f;
        GetComponent<Orbits>().angle = 0.3f;
        transform.position = new Vector3(0, 0, 0);
        GetComponent<RotateAround>().angle = 82.23f;
    }
    ...
    void OnGUI() //display the name of the planet if the mouse point intersects the object
    {
        if (GetComponent<Mouse>().intersection == true)
            GUI.Box(new Rect(GetComponent<Mouse>().twoD.x, Screen.height - GetComponent<Mouse>().twoD.y, 50, 50), "Uranus");
    }
}
