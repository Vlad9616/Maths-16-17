...
public class moonVenus : MonoBehaviour
{

    public GameObject motherPlanet;  //parent planet
    public Venus motherScript;       //get access to mother planet's script
    Vector3 position;
    // Use this for initialization
    void Start ()
    {
        motherPlanet = GameObject.Find("Venus");  //find parent planet
        motherScript = motherPlanet.GetComponent<Venus>(); //ger script
	...
	void Update ()
    {
        ...
        GetComponent<Mouse>().twoD = Camera.current.WorldToScreenPoint(position);// convert object's coordinates from world space to view space
        GetComponent<Mouse>().inRange(); //test the distance between the mouse point and object
      
    }

    void OnGUI()
    {
        //test if the mouse point intersects the object. If it is true than display the name of the object
        if (GetComponent<Mouse>().intersection == true)
            GUI.Box(new Rect(GetComponent<Mouse>().twoD.x, Screen.height - GetComponent<Mouse>().twoD.y, 75, 50), "Venus Moon");
    }
}
