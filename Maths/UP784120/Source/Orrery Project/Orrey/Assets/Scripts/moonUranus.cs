...
public class moonUranus : MonoBehaviour {

    public GameObject motherPlanet; //parent planet
    public Uranus motherScript;     //get access to mother planet's script
    Vector3 position;

    // Use this for initialization
    void Start()
    {
        motherPlanet = GameObject.Find("Uranus");  //find parent planet
        motherScript = motherPlanet.GetComponent<Uranus>(); //get script
    ...
    void OnGUI()  //display the name of the planet if the mouse point intersects the object
    {
        ...
        if (GetComponent<Mouse>().intersection == true)
            GUI.Box(new Rect(GetComponent<Mouse>().twoD.x, Screen.height - GetComponent<Mouse>().twoD.y, 75, 50), "Uranus Moon");
    }
}
