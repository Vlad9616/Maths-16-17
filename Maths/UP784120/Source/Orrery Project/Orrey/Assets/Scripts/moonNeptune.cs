...
public class moonNeptune : MonoBehaviour {

    public GameObject motherPlanet;  //parent planet
    public Neptune motherScript;     //get access to mother planet's script
    Vector3 position;

    // Use this for initialization
    void Start()
    {
        motherPlanet = GameObject.Find("Neptune");  //find parent planet
        motherScript = motherPlanet.GetComponent<Neptune>(); //get script
    ...
    void OnGUI() //display the name of the planet if the mouse point intersects the object
    {
        ...
        if (GetComponent<Mouse>().intersection == true)
            GUI.Box(new Rect(GetComponent<Mouse>().twoD.x, Screen.height - GetComponent<Mouse>().twoD.y, 75, 50), "Neptune Moon");
    }
}
